...
export const Card = () => {
  ...
  const chars = characteristics?.reduce((acc, obj) => {
    acc[obj[0]] = obj[1];
    return acc;
  }, {});
          ...
          <div className={style.info}>
            ...
            <div className={style.chars}>
              ...
              <ul className={style.list}>
                {/* {Object?.keys(chars)?.map((item, index) => (
                  <li key={index} className={style.listItem}>
                    <span className={style.point}>{item}</span>
                    <span className={style.value}>{chars[item]}</span>
                  </li>
                ))} */}
              </ul>
            </div>
